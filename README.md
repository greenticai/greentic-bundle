...
- Remote non-file catalogs are centralized behind one client seam and now support a defined GHCR shorthand:
  `ghcr://packs/well-known.json[:tag|@sha256:...]` maps to `ghcr.io/greenticai/packs/well-known.json[:tag|@sha256:...]` and defaults to `:latest` when no tag/digest is supplied.
...
- The checked-in source for the default public catalog is `packs/well-known.json`.
- Pushing a change to `packs/well-known.json` on `main` or `master` now runs `.github/workflows/catalog.yml`, which publishes both `ghcr.io/greenticai/packs/well-known.json:sha-<commit>` and `:latest`.
